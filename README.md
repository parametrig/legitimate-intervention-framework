...
## Overview
This repository contains the data pipeline, research methodology, and manuscript for the Native Compliance Framework (NCF). The project quantifies and analyzes systemic risk in decentralized finance, proposing automated "Ex Ante" (Circuit Breaker) and "Ex Post" (Governance Freeze) mechanisms.

**Key Stats (Dec 2025):**
- **759 exploits** analyzed (2014-2025)
- **$91.3 billion** in total losses
- **440 NCF-addressable** incidents (~$10.9B)
...
│   └── refined/      # final datasets (ncf_exploits_final.csv)
├── scripts/
│   ├── core/         # data pipeline (parse, dedupe, filter)
│   └── analysis/     # ncf_charts.ipynb (single source of truth)
...
**Outputs:**
- `visualizations/*.png` 
- `data/refined/ncf_exploits_cleaned.csv` (cleaned dataset)
- `data/refined/ncf_stats.json` (rich statistics with sources)
